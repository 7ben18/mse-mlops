...
- Analysis-specific derived CSVs may exist separately, but they are not required by `src/mse_mlops/data_processing.py`.
...
- Splits: the source does not provide official splits. Project splits are generated by `src/mse_mlops/data_processing.py` and `make data-split`.
